...
def registrar_Empleado(request):
    ...
    if Empleado.objects.filter(nombre_completo=nombre_completo).exists():
...
def obtener_empleado_nombre(request, nombre):
    try:
        empleado = Empleado.objects.get(nombre_completo=nombre)
...
def obtener_empleado_por_usuario(request, id_usuario):
    try:
        empleado = Empleado.objects.get(id_usuario=id_usuario)
        serializer = EmpleadoSerializer(empleado)
        return Response(serializer.data, status=status.HTTP_200_OK)
    except Empleado.DoesNotExist:
        return Response({'error': 'Empleado no encontrado'}, status=status.HTTP_404_NOT_FOUND)
